"""Materialisierung des versionierten P-K1-/A-M4-Pflicht-Fixtures.
...
FIXTURE_PFAD = REPO_ROOT / "tests" / "fixtures" / "pk1_am4_minimal" / "fixture.json"
...
def lade_pk1_fixture() -> O3Fixture:
    ...
    if not FIXTURE_PFAD.is_file():
        raise AssertionError(
            f"versioniertes P-K1-/A-M4-Pflicht-Fixture fehlt: {FIXTURE_PFAD}"
        )
    roh = json.loads(FIXTURE_PFAD.read_text(encoding="utf-8"))
    if roh.get("schema_version") != 1:
        raise AssertionError("P-K1-/A-M4-Fixture hat nicht schema_version 1")
    if roh.get("generation") != O3_GENERATION:
        raise AssertionError(
            f"P-K1-/A-M4-Fixture muss {O3_GENERATION!r} beschreiben"
        )

    parameter = roh.get("parameter")
    if not isinstance(parameter, dict):
        raise AssertionError("P-K1-/A-M4-Fixture ohne Parameterobjekt")
    if set(parameter) != set(BEKANNTE_PARAMETER):
        raise AssertionError(
            "P-K1-/A-M4-Fixture deckt den Parametervertrag nicht exakt: "
    ...
    if not quelle.is_file():
        raise AssertionError(f"Quelle des P-K1-/A-M4-Fixtures fehlt: {quelle}")
    ist_hash = sha256(quelle.read_bytes()).hexdigest()
    if ist_hash != quelle_daten.get("sha256"):
        raise AssertionError(
            "Quelle des P-K1-/A-M4-Fixtures weicht vom versionierten SHA-256 ab: "
            f"{ist_hash}"
        )
    ...
    if not isinstance(erwartung, dict):
        raise AssertionError("P-K1-/A-M4-Fixture ohne Erwartungsobjekt")
    return O3Fixture(
        fall=str(roh["fall"]),
