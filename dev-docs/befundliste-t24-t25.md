...
| **T25-03 Teil (b)** | ERLEDIGT. Entscheid 3 wurde am 2026-09-15 getroffen und gebaut: `abnahmebericht` fuehrt neben `PB1_VOLLPROFIL` jetzt `PB1_VOLLPROFIL_SCHICHT = {schichten, verankerung}` und schlaegt es dem Profil zu, sobald der Fall eine materialisierte `schichten.parquet` fuehrt (abnahmebericht.py, `vollprofil |= PB1_VOLLPROFIL_SCHICHT`). Traegt die Messung bei N-01: rund 13.700 EUR Deckungskapital je Vertrag Unterschied, wenn A-M4 einen Bestand abnimmt, dessen Korrekturschicht P-B1 nie gelesen hat. Die Bedingung fragt das DATEISYSTEM des Falls, nicht den Beleg — der Produzent kann sich die Pflicht nicht selbst wegreden. Abgesichert durch Mutationsprobe (`PB1_VOLLPROFIL_SCHICHT` leeren -> rot) und die Gate-Vertrags-Ratsche in test_betrieb_drift_n01.py. **Diese Zeile stand bis zum 2026-09-16 faelschlich auf OFFEN** — geglaubt statt am Code nachgesehen. |
...
| **T24-01** | ERLEDIGT, beide Schritte. (a) 41eea02: Aufraeumen bei unklarer Praemisse loescht nichts mehr, ein vorhandener Abschluss wird nachgerechnet statt geglaubt, die Protokollzeile liegt im Fehlerrahmen. (b) 71b9021: Write-Ahead-Marker plus Ruecknahme — der Lauf wird umkehrbar, und der Standwechsel laesst seinen Vorgaenger liegen, bis der Publish durch ist. Klassen-Test faehrt alle vier Naehte einzeln an. |
| **T25-05** | ERLEDIGT, beide Haelften. (a) war 23869c3/31c1caf; (b) 50323d5: `gates._common.Eingangsbindung` ist die gemeinsame Tuer, die drei offenen Producer binden ihre Eingaben, und die Bindung merkt sich ihre Lesungen — damit prueft die Kettenpruefung eines fremden Belegs GENAU DIE Bytes, die der Lauf verarbeitet. |
| **T24-04 Teil 2 (Block C)** | ERLEDIGT. Entscheid des Maintainers 2026-09-16: externer Anker plus Zeichnung des Exports. Gebaut ist der Anker (`models/anker.py`, Paketschema 4): Der Export legt den Hash der letzten Protokollzeile im Fall-Datenraum ab, das Paket NENNT den Satz, der Konsument prueft dagegen. Der Klassen-Test baut ein in sich stimmiges gefaelschtes Paket — es besteht jede paketinterne Pruefung und faellt am Anker. Die Zeichnung des Exports ist als offener Punkt vermerkt: Sie braucht eine Festlegung, wer zeichnet. |
