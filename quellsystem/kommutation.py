"""Kommutationsspalten D/N/C/M — der Rechenkern des QUELLSYSTEMS.

EINGEFRORENE Kopie (2026-08-31, Stand f0938c7) des Zweitkerns
``rechner_pipeline.kommutationskern`` — exakt das VBA-Modul ``mGWerte``
des historischen Quell-Workbooks (gerundete l_x-Kette). Excel bleibt der
Tarifrechner der Quelle; diese Kopie rechnet dieselbe Arithmetik in
Python, damit Bestandsfuehrung und Lieferungserzeugung ohne den
Windows-Umweg laufen. KEIN Import aus ``rechner_pipeline`` — der
Quellcode des Quellsystems ist fuer das Migrationsprojekt unerreichbar,
und Zielsystem-Aenderungen duerfen nicht durchsickern (README).
"""
...
from quellsystem.konventionen import MAX_ALTER, RADIX, excel_round
from quellsystem.tafeln import (  # noqa: F401 — Re-Export-Kompat
